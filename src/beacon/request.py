...
class RequestLogger:
    ...
    def log_request(
        ...
        **kwargs,
        ...
        if status_code >= 500:
            self.logger.error(f"HTTP {method} {path} - {status_code} ({duration:.3f}s)", extra=log_data)
        elif status_code >= 400:
            self.logger.warning(f"HTTP {method} {path} - {status_code} ({duration:.3f}s)", extra=log_data)
        else:
            self.logger.info(f"HTTP {method} {path} - {status_code} ({duration:.3f}s)", extra=log_data)
