...
def basic_logging_example():
    ...
    logger.info(
        "User action",
        extra={"user_id": "123", "action": "login", "ip_address": "192.168.1.1"},
    )
...
def structured_logging_example():
    ...
    logger.error(
        "Database connection failed", error_code=500, database="postgres", retry_count=3
    )
...
def performance_monitoring_example():
    ...
    with performance_tracker(
        "database_query", {"table": "users", "operation": "select"}
    ):
...
def request_logging_example():
    ...
    for request in requests:
        ...
        print(
            f"  Logged {request['method']} {request['path']} - {request['status_code']}"
        )
...
def training_logging_example():
    ...
    training_logger.log_training_start(
        ...
        dataset_info={"name": "imdb", "size": 25000, "split": "train"},
    )
    ...
    for epoch in range(3):
        for step in range(0, 100, 25):
            ...
            training_logger.log_training_step(
                ...
                epoch=epoch,
                loss=loss,
                metrics={"accuracy": accuracy, "learning_rate": 0.001},
            )
